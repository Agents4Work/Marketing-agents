...
function checkVideoScriptwriterFile() {
  const filePath = path.join('client', 'src', 'data', 'agents', 'video-scriptwriter.ts');
...
function checkAgentExports() {
  const filePath = path.join('client', 'src', 'data', 'agents', 'index.ts');
  if (fs.existsSync(filePath)) {
...
function checkAgentInSampleData() {
  const filePath = path.join('client', 'src', 'pages', 'AgentCategoryPage.tsx');
  if (fs.existsSync(filePath)) {
    const fileContent = fs.readFileSync(filePath, 'utf-8');
